...
/// <summary>
/// Allows for custom labels and adjustment of those labels for Vector fields.<br/><br/>
/// Minorly edited from Unity Answers user "Hellium" via<br/>
/// <see href="http://answers.unity.com/answers/1573684/view.html"/> by<br/>
/// Patrick Mitchell @ <see href="https://patrickode.github.io/"/>.
/// </summary>
[CustomPropertyDrawer(typeof(VectorLabelsAttribute))]
public class VectorLabelsPropertyDrawer : PropertyDrawer
{
    private static readonly GUIContent[] defaultLabels = new GUIContent[]
    {
    ...
    private T[] DrawFields<T>(Rect rect, T[] vector, string mainLabel,
        System.Func<Rect, GUIContent, T, T> fieldDrawer, VectorLabelsAttribute vectorLabels)
    {
        T[] result = vector;
        ...
        else
        {
            ...
            //Width equals full rect sans main label divided by number of coords needed.
            //  Also subtract half the padding between those coords for each gap in coords.
            fieldRect.width = (rect.width - mainLabelRect.width) / vector.Length
                - vectorLabels.PaddingBtwnCoords / 2 * (vector.Length - 1);
        }

        EditorGUI.LabelField(mainLabelRect, mainLabel);

        var originalAlign = EditorStyles.label.alignment;
        ...
        for (int i = 0; i < vector.Length; i++)
        {
            ...
            EditorGUIUtility.labelWidth = labelSize.x + vectorLabels.LabelPadding;

            result[i] = fieldDrawer(fieldRect, label, vector[i]);

            fieldRect.x += fieldRect.width + (i < vector.Length - 1
                ? vectorLabels.PaddingBtwnCoords
                : 0);
        }

        EditorStyles.label.alignment = originalAlign;

//This define used to be wrapped in a #if !UNITY_2019_3_OR_NEWER
...
namespace Cinemachine
{
    ...
    [SaveDuringPlay]
    public class Cinemachine3rdPersonFollowMultiTag : CinemachineComponentBase
    {
        ...
        public Bewildered.UHashSet<TagString> IgnoreTags = new();

        #region Added Stuff for Multi Tag Ignore
        private static RaycastHit[] hitBuffer = new RaycastHit[16];
        ...
        static void DestroyScratchCollider()
        {
            if (s_ScratchColliderGameObject != null)
            {
                ...
                //These used to be "DestroyObject" but said method is deprecated
                Destroy(s_ScratchColliderGameObject.GetComponent<Rigidbody>());
            }
            Destroy(s_ScratchCollider);
            Destroy(s_ScratchColliderGameObject);
            ...
            float cameraRadius, ref float collisionCorrection)
        {
            ...
            //Originally had a "RuntimeUtility" before it; Patrick yoinked that (see method below) and edited it to work
            //with multiple tags B)
            if (SphereCastIgnoreTag(
                root, cameraRadius, dir, out RaycastHit hitInfo,
                len, CameraCollisionFilter, IgnoreTags))
            {
        ...
        /// <summary>
        /// Perform a sphere cast, but pass through objects with a given tag. Edited from 
        /// <see href="https://github.com/Unity-Technologies/com.unity.cinemachine/blob/main/com.unity.cinemachine/Runtime/Core/RuntimeUtility.cs"/>
        /// </summary>
        ...
        /// <param name="ignoreTags">Tags to ignore</param>
        ...
        public static bool SphereCastIgnoreTag(
            ...
            int layerMask, in Bewildered.UHashSet<TagString> ignoreTags)
        {
            int closestHit = -1;
            int numPenetrations = 0;
            float penetrationDistanceSum = 0;
            int numHits = Physics.SphereCastNonAlloc(
                rayStart, radius, dir, hitBuffer, rayLength, layerMask,
                QueryTriggerInteraction.Ignore);
            for (int i = 0; i < numHits; ++i)
            {
                var h = hitBuffer[i];
                if (ignoreTags.Count > 0 && ignoreTags.Contains(h.collider.tag))
                    continue;
                ...
                if (closestHit < 0 || h.distance < hitBuffer[closestHit].distance)
                {
                    closestHit = i;
                }
            }
            ...
            if (numPenetrations > 1)
            {
                ...
                {
                    var h = hitBuffer[s_PenetrationIndexBuffer[i]];
            ...
            if (closestHit >= 0)
            {
                hitInfo = hitBuffer[closestHit];
                if (numHits == hitBuffer.Length)
                    hitBuffer = new RaycastHit[hitBuffer.Length * 2]; // full! grow for next time
